...
public interface IKeycloakService
{
    ...
    Task<KeycloakClient?> GetClientAsync(string clientId, string? userId = null);
    ...
    Task<bool> DeleteClientAsync(string clientId, string? userId = null);
}
...
public class KeycloakService : IKeycloakService
{
    ...
    public async Task<List<KeycloakClient>> GetClientsAsync(string? userId = null)
    {
        ...
        // Filtrer par propriétaire si userId est fourni
    ...
    public async Task<KeycloakClient?> GetClientAsync(string clientId, string? userId = null)
    {
    ...
    {
        var client = await GetClientAsync(clientId, userId);
    ...
    public async Task<bool> DeleteClientAsync(string clientId, string? userId = null)
    {
        // Vérifier d'abord que l'utilisateur possède ce client
        var client = await GetClientAsync(clientId, userId);
        if (client == null) return false;

        await SetAuthorizationHeaderAsync();

        var response = await _httpClient.DeleteAsync($"{_keycloakUrl}/admin/realms/{_realm}/clients/{client.id}");
